...
- `search_emails` takes a Gmail-style query string (`from:x in:Inbox/Work is:unread newer_than:7d`). `list_address_books` lists the address books the contacts scope can see.
- Optional passthrough: set `FASTMAIL_MCP_TOKEN` to an API token of type MCP and the tools Fastmail's own MCP server has that JMAP cannot reach appear with the prefix `official_`: Notes, email memos, the company directory, calendar RSVP and compose. Calendar tools come along only when CalDAV is not configured. Without the token nothing changes.
